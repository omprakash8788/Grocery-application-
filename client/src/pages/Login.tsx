...
const Login = () => {
  ...
  /*
     so bydefault isLogin true then we will display login form else we will display signup form.
     */
  ...
  const handleSubmit = async (e: React.SubmitEvent) => {
    e.preventDefault();
    setLoading(true);
    setTimeout(() => (window.location.href = "/"), 1000); // It will simply redirect user to home page
  };
    ...
    <div className="min-h-screen flex">
      ...
      <div className="hidden lg:flex lg:w-1/2 bg-app-green relative items-center justify-center">
        <img
          ...
          alt="img"
          className="h-full absolute inset-0 object-cover bg-center opacity-10"
        />
        <div className="relative text-center px-12">
          <h2 className="text-4xl font-semibold text-white mb-4">
            Welcome back to BlinkCart
          </h2>
          <p className="text-white/60 font-serif text-xl max-w-sm mx-auto">
            Fresh groceries and organic product
          </p>
        </div>
      </div>
              ...
              <label className="text-sm flex flex-col gap-1">
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full p-11 pr-4 py-3 text-sm bg-white rounded-xl border not-focus:border-app-border transition-all"
                  />
                </div>
              </label>
            )}
